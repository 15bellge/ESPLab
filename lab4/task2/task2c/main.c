...
void filePrint(linux_dirent* file, int debug_mode, int isPrefix, int isAppend, char d_type){
...
void addressesPrint(){
    char* start_address = itoa((int)code_start);
    system_call(SYS_WRITE,STDOUT, "Address of code_start: ", 23);
    system_call(SYS_WRITE,STDOUT, start_address, strlen(start_address));
    system_call(SYS_WRITE,STDOUT, "\n", 1);;
    char* end_address = itoa((int)code_end);
    system_call(SYS_WRITE,STDOUT, "Address of code_end: ", 21);
    system_call(SYS_WRITE,STDOUT, end_address, strlen(end_address));
    system_call(SYS_WRITE,STDOUT, "\n", 1);
        ...
        if(strcmp(file->d_name, ".") != 0 && strcmp(file->d_name, "..") != 0){
            if(isPrefix || isAppend) {
                if (strncmp(file->d_name, prefix, strlen(prefix)) == 0) {
                    filePrint(file, debug_mode, isPrefix, isAppend, d_type);
                }
            }
            else if(!isPrefix) {
                filePrint(file, debug_mode, isPrefix, 0, d_type);
            }
        }
        i+=file->reclen;
    }
